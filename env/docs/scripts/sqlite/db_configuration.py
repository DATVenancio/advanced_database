...
def create_tables():
    ...
    db_cursor.execute("DROP TABLE IF EXISTS directors;")
    db_cursor.execute('''
    ...
    with open("../imdb-tiny/writers.csv", 'r', newline='') as file:
        ...
        for line in reader_csv:
            ...
            db_cursor.execute('''INSERT OR REPLACE INTO writers (mid,pid) 
                           ...
                           (mid,pid))
    #writers
    with open("../imdb-tiny/directors.csv", 'r', newline='') as file:
        reader_csv = csv.reader(file)
        next(reader_csv)  #jump first line
        for line in reader_csv:
            #print(line)
            mid,pid= line
            db_cursor.execute('''INSERT OR REPLACE INTO directors (mid,pid) 
                              VALUES (?, ?)''', 
                           (mid,pid))
